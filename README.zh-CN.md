...
打包好的 `.exe` 内置的是 **纯 CPU 版** PyTorch,所以不会用 GPU(把 CUDA 打进去
会让体积大好几个 GB)。如果你有 NVIDIA 显卡、想用 GPU,请改为**从源码运行**:

- **Windows 一键:** 双击 [`run_gpu.bat`](run_gpu.bat)。首次运行会创建本地
  `.venv` 并安装 CUDA 版 PyTorch(一次性,较大);之后启动很快并自动用 GPU。
- **手动:**
...
检测器会自动检测 CUDA(`torch.cuda.is_available()`),有显卡时用 GPU,否则回退到
CPU。也可以用 `DetectionConfig(device="cuda:0")` 手动指定设备。
...
批量模式使用**高召回预设**（`DetectionConfig.high_recall()`），专为人多/重叠场景
调过:更大的模型(`yolov8m`)、更高的 NMS IoU 阈值(避免把重叠的人合并成一个)、
更低的置信度阈值、更小的最小框面积、更多的抽帧数量。它的取向是**尽量不漏人**、
置信度更稳——代价是偶尔会有误检,而且处理更慢(尤其在纯 CPU 上)。
...
- **内存预览** —— 标注后的预览图保存在内存中、用弹窗显示,图形界面不再往磁盘写图片文件。
- **更安全的抽帧定位、类型注解和文档字符串**。
